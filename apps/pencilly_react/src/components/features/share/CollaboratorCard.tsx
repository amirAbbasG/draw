...
const CollaboratorCard: FC<IProps> = ({
  ...
  return (
    <div
      className={cn(
        "row gap-2 py-1.5 px-3 rounded  w-full  bg-muted hover:bg-muted-dark transition-colors ",
    ...
    >
      ...
      <div className="relative">
        <UserAvatar
          className="size-7 text-[8px]  border-2 p-[2px] select-none "
      ...
      <div className="flex-1 min-w-0 col gap-0.5">
        <div className="flex items-center gap-2">
          ...
          <div className="row gap-2 text-sm text-foreground-light border-s ps-2">
            {isCollabOwner ? (
              <span className="row gap-1">
                <AppIcon
                  icon="hugeicons:shield-01"
                  className="h-3 w-3 mb-[1px]"
                />
                {t("role_owner")}
              </span>
            ) : (
              <span className="row gap-1">
                <AppIcon
                  icon={canEdit ? sharedIcons.edit : sharedIcons.view}
