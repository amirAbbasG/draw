...
import { updateObject } from "@/stores/zustand/object/actions";
import { setTransformMode } from "@/stores/zustand/three/actions";
...
export function CustomTransformControls({
  object,
  onDeselect,
}: CustomTransformControlsProps) {
  const [transformMode, isDeleting] = useThreeStore(
    useShallow(state => [state.transformMode, state.isDeleting]),
  );
  ...
  useEffect(() => {
    ...
    const checkObjectInScene = () => {
      ...
      if (!isInScene) {
        console.warn(
          `Selected object ${object.uuid} is not in scene, cannot attach controls`,
        );
  ...
  const handleObjectChange = useCallback(() => {
    ...
    if (
      window.__lastTransformUpdate &&
      Date.now() - window.__lastTransformUpdate < 50
    ) {
      return;
    }
