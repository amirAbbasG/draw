...
const createMeshFromStored = (data: StoredObject): THREE.Mesh => {
  ...
  if (originalObject instanceof THREE.Mesh) {
    ...
    console.log(
      `Created mesh from original object: id=${data.id}, uuid=${mesh.uuid}`,
    );
    return mesh;
  }

  // Try to use the original geometry from references
  const originalGeometry = window.__objectReferences?.get(
    `${objectId}_geometry`,
  ) as THREE.BufferGeometry;
  const originalMaterial = window.__objectReferences?.get(
    `${objectId}_material`,
  ) as THREE.Material;
...
const createGroupFromStored = (data: StoredObject): THREE.Group => {
  ...
  if (originalGroup instanceof THREE.Group) {
    ...
    console.log(
      `Created group from original object: id=${data.id}, uuid=${group.uuid}`,
    );
    return group;
  }
  ...
  if (data.children && data.children.length > 0) {
    console.log(
      `Creating ${data.children.length} children for group ${data.id}`,
    );
  ...
  console.log(
    `Created group from storage: id=${data.id}, uuid=${group.uuid}, children: ${group.children.length}`,
  );
...
const createObject3DFromStored = (data: StoredObject): THREE.Object3D => {
  ...
  const originalObject = window.__objectReferences?.get(
    data.id,
  ) as THREE.Object3D;

  if (
    originalObject &&
    !(originalObject instanceof THREE.Mesh) &&
    ...
    console.log(
      `Created object3D from original object: id=${data.id}, uuid=${obj.uuid}`,
    );
    return obj;
  }
  ...
  if (data.children && data.children.length > 0) {
    console.log(
      `Creating ${data.children.length} children for object3D ${data.id}`,
    );
    data.children.forEach(childData => {
      let child: THREE.Object3D | null = null;
      if (childData.type === "mesh") {
  ...
  console.log(
    `Created generic object3D from storage: id=${data.id}, uuid=${obj.uuid}`,
  );
    ...
    if (threeObjects.length === 0) {
      objectRefs.current.forEach(obj => {
        if (scene.getObjectById(obj.id)) {
          console.log(
            `Removing object from scene (no stored objects): ${obj.userData.id || obj.uuid}`,
          );
          scene.remove(obj);
        }
      });
