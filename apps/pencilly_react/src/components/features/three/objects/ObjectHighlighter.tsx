...
import { removeObject } from "@/stores/zustand/object/actions";
import {
  setIsDeleting,
  setSelectedObject,
...
export function ObjectHighlighter({
  onObjectSelected,
  excludeObjects = [],
}: ObjectHighlighterProps) {
  ...
  const [isUIFocused, selectedObject] = useThreeStore(
    useShallow(state => [state.isUIFocused, state.selectedObject]),
  );

  // Store original materials for selected objects
  const originalMaterials = useRef(
    new Map<THREE.Object3D, THREE.Material | THREE.Material[]>(),
  );

  // Store original materials for hovered objects
  const hoveredMaterials = useRef(
    new Map<THREE.Object3D, THREE.Material | THREE.Material[]>(),
  );
  ...
  useEffect(() => {
    ...
    const applyHoverTint = (object: THREE.Object3D) => {
      if (
        ...
        !originalMaterials.current.has(object)
      ) {
        // Don't apply hover to selected objects
        // Store original material if not already stored
    ...
    if (isShiftKeyPressed) {
      ...
      if (hoveredObject.children && hoveredObject.children.length > 0) {
        ...
        const directChildren = hoveredObject.children.filter(
          child =>
            (child.userData?.isUserCreated ||
              child.userData?.isSerializedFromCode) &&
        ...
        if (directChildren.length > 0) {
          ...
          const childIntersects = childRaycaster.intersectObjects(
            directChildren,
            true,
          );
          ...
          if (childIntersects.length > 0) {
            ...
            if (intersectedDirectChild) {
              ...
              const directGroupChild = directChildren.find(
      ...
      while (currentObj.parent && !(currentObj.parent instanceof THREE.Scene)) {
        if (
          (currentObj.parent.userData?.isUserCreated ||
            currentObj.parent.userData?.isSerializedFromCode) &&
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      try {
        ...
        if (window.__transformControlsActive) {
          console.log(
            "ObjectHighlighter: Skipping click because TransformControls is active",
          );
          return;
        }
        ...
        if (hoveredObject && onObjectSelected && !isUIFocused) {
          console.log(
            "ObjectHighlighter: click on object",
            hoveredObject.userData?.name || "unnamed",
            "uuid:",
            hoveredObject.uuid,
          );
          ...
          if (isShiftKeyPressed) {
            ...
            console.log(
              ...
              hoveredObject.userData?.name || "unnamed",
              "type:",
              hoveredObject instanceof THREE.Group
                ? "Group"
              ...
              if (directChildren.length > 0) {
                ...
                if (childIntersects.length > 0) {
                  ...
                  if (intersectedDirectChild) {
                    ...
                    targetObject = intersectedDirectChild;
                    console.log(
                      "Raycast hit direct child:",
                      targetObject.userData?.name || "unnamed",
                    );
                  } else {
                    // Fallback: First try to find a Group among direct children
                    const directGroupChild = directChildren.find(
                    ...
                    if (directGroupChild) {
                      targetObject = directGroupChild;
                      console.log(
                        "Fallback to direct GROUP child:",
                        targetObject.userData?.name || "unnamed",
                      );
                    } else {
                      // Otherwise just pick the first direct child
                ...
                console.log(
                  "No suitable direct children found, keeping hovered object",
                );
              }
            } else {
              console.log(
                "Hovered object has no children, keeping it as target",
              );
            }
            ...
            if (
              targetObject &&
              (targetObject.userData?.isUserCreated ||
                targetObject.userData?.isSerializedFromCode)
            ) {
              console.log(
                "Final shift+click selection:",
                targetObject.userData?.name || "unnamed",
                "uuid:",
                targetObject.uuid,
              );
          ...
          while (
            currentObj.parent &&
            !(currentObj.parent instanceof THREE.Scene)
          ) {
            if (
              (currentObj.parent.userData?.isUserCreated ||
                currentObj.parent.userData?.isSerializedFromCode) &&
              ...
              found = true;
              console.log(
                `Using parent group for selection: ${targetObject.userData?.name || "unnamed"}`,
              );
              break;
            }
            currentObj = currentObj.parent;
          ...
          const selectedObjectId =
            selectedObject?.userData?.id || selectedObject?.uuid;
          ...
          console.log(
            `Selecting object: id=${objectId}, type=${targetObject instanceof THREE.Group ? "Group" : "Mesh"}`,
          );
    ...
    const handleDoubleClick = () => {
      try {
        ...
        if (window.__transformControlsActive) {
          console.log(
            "ObjectHighlighter: Skipping double-click because TransformControls is active",
          );
          return;
        }
    ...
    try {
      ...
      if (intersects.length > 0) {
        ...
        while (
          ...
          if (
            (currentObj.parent.userData?.isUserCreated ||
              currentObj.parent.userData?.isSerializedFromCode) &&
            (currentObj.parent instanceof THREE.Group ||
              currentObj.parent instanceof THREE.Object3D)
          ) {
        ...
        if (hoveredObject !== topLevelObject) {
          ...
          console.log(
            "Set hovered object:",
            topLevelObject.userData?.name || "unnamed",
            "type:",
            topLevelObject instanceof THREE.Group
              ? "Group"
              : topLevelObject instanceof THREE.Mesh
                ? "Mesh"
                : "Object3D",
          );
        }
...
function isDescendantOf(
  ...
  potentialAncestor: THREE.Object3D,
): boolean {
