...
import { CustomTransformControls } from "@/components/features/three/objects/CustomTransformControls";
import { ObjectHighlighter } from "@/components/features/three/objects/ObjectHighlighter";
import {
  setSelectedObject,
  setTransformMode,
...
export function MeshCreator() {
  const [selectedObject, isDeleting] = useThreeStore(
    useShallow(state => [state.selectedObject, state.isDeleting]),
  );
  ...
  useEffect(() => {
    ...
    console.log(
      `Looking for selected object: uuid=${selectedId}, userData.id=${selectedUserDataId}`,
    );
    ...
    if (foundObject) {
      const objectName =
        (foundObject as THREE.Object3D).userData?.name || "unnamed";
      console.log(`Found object in scene: ${objectName}`);

      // Check if the object is actually still in the scene hierarchy
      let isInScene = false;
      let currentParent: THREE.Object3D | null = (foundObject as THREE.Object3D)
        .parent;
      ...
      if (!isInScene) {
        console.warn(
          `Object ${objectName} (${selectedId}) was found but not in scene hierarchy`,
        );
      ...
      if (!objWithProps.userData.doubleClickHandler) {
        objWithProps.userData.doubleClickHandler = () => {
          const { transformMode } = useThreeStore.getState();
  ...
  const handleObjectSelected = (object: THREE.Object3D | null) => {
    ...
    if (isShiftKeyPressed) {
      ...
      console.log(
        "Shift key pressed, selecting:",
        object.userData?.name || "unnamed",
      );

      if (
        object.userData?.isUserCreated ||
        object.userData?.isSerializedFromCode
      ) {
  ...
  return (
    <>
      ...
      <ObjectHighlighter onObjectSelected={handleObjectSelected} />

      {selectedInstance &&
        (selectedInstance.userData?.isUserCreated ||
          selectedInstance.userData?.isSerializedFromCode) && (
