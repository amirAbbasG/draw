...
function FirstPersonController() {
  ...
  const [isUIFocused, isCodeEditorOpen, settings] = useThreeStore(
    useShallow(state => [
      state.isUIFocused,
  ...
  const { speed, sensitivity } = settings;
  ...
  useEffect(() => {
    ...
    if (isTouchDevice.current) {
      ...
      joystickContainer.style.left = "100px"; // Position further from the left
      ...
      rotationJoystickContainer.style.border =
        "2px solid rgba(100, 100, 100, 0.8)"; // Darker border
  ...
  useFrame((_, delta) => {
    ...
    if (
      ...
      if (movement.forward)
        velocity.current.add(forward.multiplyScalar(frameSpeed));
      if (movement.backward)
        velocity.current.sub(forward.multiplyScalar(frameSpeed));
      if (movement.right)
        velocity.current.add(right.multiplyScalar(frameSpeed));
