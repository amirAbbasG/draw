...
interface CallGridProps {
    ...
    maxTiles?: number;
    /** Whether the LOCAL user is currently screen-sharing (from LiveKit state) */
...
const CallGrid: FC<CallGridProps> = ({
    ...
    // Use the reliable isLocalScreenSharing prop (from LiveKit hook state) to
    // determine if the current user is the sharer. We never render the live
    // screen-share <video> for the local sharer because it would capture itself
    // and create an infinite mirror effect (even when sharing the full monitor).
    const shouldShowScreenVideo =
        !!screenSharer?.screenTrack && !isLocalScreenSharing;

    useEffect(() => {
        const el = screenVideoRef.current;
        if (!el || !shouldShowScreenVideo || !screenSharer?.screenTrack) return;
        el.srcObject = new MediaStream([screenSharer.screenTrack]);
        el.play().catch(() => {});
        return () => {
            if (el) el.srcObject = null;
        };
    }, [shouldShowScreenVideo, screenSharer?.screenTrack]);
                ...
                <div className="flex-1 min-h-0 rounded-lg overflow-hidden bg-foreground/5 relative">
                    {isLocalScreenSharing ? (
                        <div className="h-full w-full flex items-center justify-center bg-foreground/5">
                            <div className="col items-center gap-2 text-muted-foreground">
                                <svg
                                ...
                                <AppTypo  className="text-muted-foreground">
                                    {t("screen_share")}
                                </AppTypo>
                            </div>
                        </div>
                    ...
                    <div className="absolute bottom-2 start-2 flex items-center gap-1 px-2 py-1 rounded-md bg-background/80 backdrop-blur-sm">
                        <AppTypo variant="xs">
                            {isLocalScreenSharing ? t("me") : screenSharer.name} -{" "}
                            {t("screen_share")}
                        </AppTypo>
                    </div>
                </div>
