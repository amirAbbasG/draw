...
import { initializeScene } from "@/components/features/draw/local-storage";
import {
  RECONNECT_ATTEMPTS,
...
export const useYjsSession = (drawAPI: DrawAPI) => {
  ...
  const destroySession = useCallback(() => {
    ...
    if (drawAPI) {
      void initializeScene().then(({ elements, appState }) => {
        drawAPI.updateScene({
          elements,
