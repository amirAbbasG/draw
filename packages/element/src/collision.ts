...
import { curveIntersectLineSegment, isPointWithinBounds, lineSegment, lineSegmentIntersectionPoints, LocalPoint, pointFrom, pointFromVector, pointRotateRads, pointsEqual, segmentsIntersectAt, vectorFromPoint, vectorNormalize, vectorScale } from "@excalidraw/math";
...
import { doBoundsIntersect, elementCenterPoint, getCenterForBounds, getCubicBezierCurveBound, getElementBounds, type Bounds } from "./bounds";
import { distanceToElement } from "./distance";
import { LinearElementEditor } from "./linearElementEditor";
import { getBoundTextElement } from "./textElement";
import { hasBoundTextElement, isFreeDrawElement, isIframeLikeElement, isImageElement, isLinearElement, isTextElement } from "./typeChecks";
import type {
  ElementsMap,
...
import { deconstructDiamondElement, deconstructLinearOrFreeDrawElement, deconstructRectanguloidElement, isPathALoop } from "./utils";
...
export const intersectElementWithLineSegment = (
  ...
  switch (element.type) {
    ...
    case "pentagon":
      return intersectPolygonWithLineSegment(element, line, 5, offset);
    case "hexagon":
      return intersectPolygonWithLineSegment(element, line, 6, offset);
    case "diamond":
...
>(
  ...
  return [
    lineSegment<Point>(top  as Point, bottomLeft  as Point),
    lineSegment<Point>(bottomLeft  as Point, bottomRight  as Point),
    lineSegment<Point>(bottomRight  as Point, top  as Point),
  ]
...
const intersectPolygonWithLineSegment = <Point extends LocalPoint | GlobalPoint>(
  ...
  for (let i = 0; i < numSides; i++) {
    ...
    const rotated = pointRotateRads(pointFrom(vx, vy), center, angle);
    vertices.push(rotated as Point);
  }

  // Create line segments for each edge
  const edges: LineSegment<Point>[] = [];
  for (let i = 0; i < vertices.length; i++) {
    const j = (i + 1) % vertices.length;
    edges.push(lineSegment<Point>(vertices[i], vertices[j]));
  }
