...
export class ElementBounds {
  ...
  private static calculateBounds(
    ...
    if (isFreeDrawElement(element)) {
      ...
      // <CHANGE> Calculate tight bounds for triangle using its 3 vertices
      const [topX, topY, bottomLeftX, bottomLeftY, bottomRightX, bottomRightY] =
        getTrianglePoints(element);

      // Convert local coordinates to global coordinates
      const globalTopX = element.x + topX;
      const globalTopY = element.y + topY;
      const globalBottomLeftX = element.x + bottomLeftX;
      const globalBottomLeftY = element.y + bottomLeftY;
      const globalBottomRightX = element.x + bottomRightX;
      const globalBottomRightY = element.y + bottomRightY;

      // Rotate each vertex around the center
      const [x1r, y1r] = pointRotateRads(
        pointFrom(globalTopX, globalTopY),
        pointFrom(cx, cy),
        element.angle,
      );
      const [x2r, y2r] = pointRotateRads(
        pointFrom(globalBottomLeftX, globalBottomLeftY),
        pointFrom(cx, cy),
        element.angle,
      );
      const [x3r, y3r] = pointRotateRads(
        pointFrom(globalBottomRightX, globalBottomRightY),
        pointFrom(cx, cy),
        element.angle,
      );

      const minX = Math.min(x1r, x2r, x3r);
      const minY = Math.min(y1r, y2r, y3r);
      const maxX = Math.max(x1r, x2r, x3r);
      const maxY = Math.max(y1r, y2r, y3r);
      bounds = [minX, minY, maxX, maxY];
    } else if (element.type === "pentagon") {
      const points = getPentagonPoints(element);
      const vertices: GlobalPoint[] = [];
      for (let i = 0; i < points.length; i += 2) {
        const globalX = element.x + points[i];
        const globalY = element.y + points[i + 1];
        const [rx, ry] = pointRotateRads(
          pointFrom(globalX, globalY),
          pointFrom(cx, cy),
          element.angle,
        );
        vertices.push(pointFrom(rx, ry));
      }
...
export const getElementLineSegments = (
  ...
  if (shape.type === "polycurve") {
    ...
    const pointsOnCurves = curves.map(curve => pointsOnBezierCurves(curve, 10));
...
export const getTrianglePoints = (element: ExcalidrawTriangleElement) => {
  ...
  // Use LOCAL coordinates (relative to 0,0), not global
  const topX = width / 2;
  const topY = 0;
  const bottomLeftX = 0;
...
export const getPentagonPoints = (element: ExcalidrawPentagonElement) => {
  const { width, height } = element;
  const cx = width / 2;
  const cy = height / 2;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const x = cx + radius * Math.cos(angle) * (width / (radius * 2));
    const y = cy + radius * Math.sin(angle) * (height / (radius * 2));
    points.push(x, y);
  }
  return points; // [x0, y0, x1, y1, x2, y2, x3, y3, x4, y4]
};

export const getHexagonPoints = (element: ExcalidrawHexagonElement) => {
  const { width, height } = element;
  const cx = width / 2;
  const cy = height / 2;
  const radius = Math.min(width, height) / 2;
  ...
  return points; // [x0, y0, x1, y1, x2, y2, x3, y3, x4, y4, x5, y5]
};
