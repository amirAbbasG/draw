...
/**
 * Moves element(s) to a specified index in the elements array.
 * @param elements - The array of elements
 * @param appState - The current app state
 * @param targetIndex - The index where elements should be moved to
 * @returns Updated elements array with elements moved to target index
 */
export const moveElementsToIndex = (
    elements: readonly ExcalidrawElement[],
    appState: AppState,
    targetIndex: number,
): ExcalidrawElement[] => {
  ...
  if (!indicesToMove.length || targetIndex < 0 || targetIndex > elements.length) {
    return [...elements];
  }

  // Filter out elements that will be moved
  const remainingElements = elements.filter(
      (_, index) => !indicesToMove.includes(index),
  );

  // Get the elements to move in their current order
  const elementsToMove = indicesToMove
      .sort((a, b) => a - b)
      .map((index) => elements[index]);

  // Insert moved elements at target index
  const result = [
  ...
  syncMovedIndices(result, targetElementsMap);
