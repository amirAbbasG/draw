...
export const getPolygonShape = <Point extends GlobalPoint | LocalPoint>(
  ...
  if (element.type === "diamond") {
    ...
    data = polygon(
      pointRotateRads(pointFrom(topX, topY), center, angle),
      pointRotateRads(pointFrom(bottomLeftX, bottomLeftY), center, angle),
      pointRotateRads(pointFrom(bottomRightX, bottomRightY), center, angle),
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const vx = x + width / 2 + radius * Math.cos(a) * (width / (radius * 2));
      const vy =
        y + height / 2 + radius * Math.sin(a) * (height / (radius * 2));
      vertices.push(pointRotateRads(pointFrom(vx, vy), center, angle));
    }
