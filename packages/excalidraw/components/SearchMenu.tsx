      ...
      if (match) {
        ...
        console.log({
          el: {
            x: matchAsElement.x,
            y: matchAsElement.y,
            width: matchAsElement.width,
            height: matchAsElement.height,
          },
            view: {
        ...
        const el = matchAsElement
        const elLeft = (el.x * zoomValue) + view.scrollX;
        const elTop = (el.y * zoomValue) + view.scrollY;
        const elRight = elLeft + (el.width * zoomValue);
        const elBottom = elTop + (el.height * zoomValue);


        const viewLeft = 0;
        const viewTop = view.offsetTop;
        const viewRight = view.width;
        const viewBottom =  view.height ;
        ...
        const isElementVisible =
          ...
          // const isElementVisible = isElementCompletelyInViewport(
          //     [matchAsElement],
          //     app.canvas.width ,
          //     app.canvas.height,
          //     {
          ...
          if (
            !isElementVisible || isTextTiny
          ) {
            let zoomOptions: Parameters<
              AppClassProperties["scrollToContent"]
            >[1];
  ...
  return (
    <div className="layer-ui__search">
      ...
      <div className="layer-ui__search-count">
        ...
        {searchMatches.items.length === 0 && !searchQuery && (
          <div
            style={{
              margin: "2rem auto",
              textAlign: "center",
...
const handleSearch = debounce(
  (
    ...
    const focusIndex =
      matchItems.findIndex(matchItem => visibleIds.has(matchItem.element.id)) ??
      null;
