...
// Shape tools to collapse on small screens (height < 900px)
const COLLAPSIBLE_SHAPES = [
  "diamond",
  "rectangle",
  "triangle",
  "pentagon",
...
export const ShapesSwitcher = ({
  ...
  const shouldCollapseShapes = windowHeight < COLLAPSE_HEIGHT_THRESHOLD;
  ...
  const SHAPE_TOOLS = COLLAPSIBLE_SHAPES.map(type => {
    const shape = SHAPES.find(shape => shape.value === type)!;
    return {
      type: shape.value,
      ...shape,
    };
  ...
  // Check if any collapsible shape is currently active
  const activeCollapsibleShape = SHAPE_TOOLS.find(
    shape => activeTool.type === shape.type,
  );
  ...
  // Render the shapes dropdown for collapsed mode
  const renderShapesDropdown = () => {
    const displayedIcon = activeCollapsibleShape?.icon || RectangleIcon;
    const isAnyShapeActive = !!activeCollapsibleShape;

    return (
      <DropdownMenu open={isShapesPopoverOpen}>
        <DropdownMenu.Trigger
          className={clsx("App-toolbar__shapes-trigger", {
            "App-toolbar__shapes-trigger--selected": isAnyShapeActive,
          })}
          onToggle={() => {
            setIsShapesPopoverOpen(!isShapesPopoverOpen);
            setAppState({ openMenu: null, openPopup: null });
          }}
          title={t("toolBar.shapes") || "Shapes"}
        >
          {displayedIcon}
        </DropdownMenu.Trigger>
        <DropdownMenu.Content
          onClickOutside={() => setIsShapesPopoverOpen(false)}
          onSelect={() => setIsShapesPopoverOpen(false)}
          className="App-toolbar__shapes-dropdown"
        >
          {SHAPE_TOOLS.map(shape => {
            const label = t(`toolBar.${shape.type}`);
            return (
              <DropdownMenu.Item
                key={shape.type}
                onSelect={() => {
                  trackEvent("toolbar", shape.type, "ui");
                  app.setActiveTool({ type: shape.type, hidePopup: false });
                }}
                icon={shape.icon}
                data-testid={`toolbar-${shape.type}`}
                selected={activeTool.type === shape.type}
              >
    ...
    <>
      {getToolbarTools(app).map(
        ({ value, icon, key, numericKey, fillable }, index) => {
          ...
          if (
            shouldCollapseShapes &&
            COLLAPSIBLE_SHAPES.includes(value as any)
          ) {
            // Render the shapes dropdown only once (at rectangle position)
            if (value === "rectangle") {
              return (
                <Fragment key="shapes-dropdown">
                  {renderShapesDropdown()}
                </Fragment>
              );
            }
            return null; // Skip other collapsible shapes
          }
          ...
          return (
            <ToolButton
              ...
              onClick={() => {
                ...
                } else if (value === app.state.activeTool.type) {
                  app.setActiveTool({
                    type: value,
                    hidePopup: !app.state.activeTool.hidePopup,
                  });
                } else {
      ...
      <DropdownMenu open={isExtraToolsMenuOpen}>
        ...
        >
          ...
          <DropdownMenu.Item
            onSelect={() => window.open( "/mermaid", "_blank")}
            icon={<Icon icon="hugeicons:workflow-square-06" />}
            data-testid="toolbar-embeddable"
          >
            {t("toolBar.advanceMermaid")}
          </DropdownMenu.Item>
