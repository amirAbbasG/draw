...
export const LIBRARY_SHAPES: LibraryShapeDefinition[] = [
    ...
    { name: "Asterisk", index: 13, icon: "asterisk" },
    ...
    { name: "Tow Way Arrow", index: 19, icon: "two-way-arrow" },
    // Row 6: Special shapes
    { name: "gemstone-style Diamond", index: 20, icon: "gemstone-diamond" },
    { name: "Lightning", index: 21, icon: "lightning" },
    { name: "Moon", index: 22, icon: "moon" },
];
...
export function createShapeFromLibrary(
    ...
    for (const element of libraryItem) {
        if (element.type === "line" && (element as ExcalidrawLinearElement).points) {
            ...
            newElements.push(newElement as unknown as ExcalidrawElement);
        } else {
...
export function getShapeIconSvg(iconName: string): string {
    ...
    switch (iconName) {
        ...
        case "dodecagon":
            return `${svgStart}<circle cx="12" cy="12" r="9" fill="#ced4da" />${svgEnd}`;
        case "trapezoid":
            return `${svgStart}<path d="M5 21 L19 21 L22 3 L2 3 Z" fill="#ced4da" transform="translate(0,24) scale(1,-1)"/>${svgEnd}`;
        case "parallelogram":
            return `${svgStart}<path d="M6 3 L22 3 L18 21 L2 21 Z" fill="#ced4da" />${svgEnd}`;
        case "chevron":
            return `${svgStart}<path d="M22 22H2V11l10-9l10 9z" fill="#ced4da" transform="translate(0,24) scale(1,-1)"/>${svgEnd}`;
        case "notched-chevron":
            return `${svgStart}<path fill="#ced4da" d="M6 19.5V4h12v15.5l-6-2.583z" transform="scale(1.2)"/>${svgEnd}`;
        case "teardrop":
            return `${svgStart}<path d="M12 2 C12 2 20 10 20 14 A8 8 0 1 1 4 14 C4 10 12 2 12 2 Z" fill="#ced4da" />${svgEnd}`;
        case "asterisk":
            return `${svgStart}<path fill="#ced4da" d="M8.7 1.7 L15.3 1.7 L14.5 8.5 L20.7 5.6 L22.8 12 L16.1 13.4 L20.7 18.4 L15.3 22.3 L12 16.3 L8.7 22.3 L3.3 18.4 L7.9 13.4 L1.2 12 L3.3 5.6 L9.5 8.5 Z" />${svgEnd}`;
        case "star-5":
        ...
        case "two-way-arrow":
            return `${svgStart}<path d="M12 2 L17 7 L14 7 L14 17 L17 17 L12 22 L7 17 L10 17 L10 7 L7 7 Z" fill="#ced4da" />${svgEnd}`;
        case "gemstone-diamond":
            return `${svgStart}<path d="M6 4 L18 4 L22 10 L12 22 L2 10 Z" fill="#ced4da" />${svgEnd}`;
        case "lightning":
            return `${svgStart}<path d="M13 2 L4 14 L11 14 L9 22 L20 10 L13 10 Z" fill="#ced4da" />${svgEnd}`;
        case "moon":
            return `${svgStart}<path fill="#ced4da" d="M12.253.335A10.09 10.09 0 0 0 8.768 8c0 4.632 3.068 8.528 7.232 9.665A9.56 9.56 0 0 1 9.742 20C4.362 20 0 15.523 0 10S4.362 0 9.742 0c.868 0 1.71.117 2.511.335"/>${svgEnd}`;
        default:
            return `${svgStart}<rect x="3" y="3" width="18" height="18" rx="1" fill="#ced4da" />${svgEnd}`;
    }
}
