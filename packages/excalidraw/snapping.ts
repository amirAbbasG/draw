import { KEYS, TOOL_TYPE } from "@excalidraw/common";
import {
  getCommonBounds,
...
import type { Bounds, MaybeTransformHandleType } from "@excalidraw/element";
import type {
  ElementsMap,
  ExcalidrawElement,
...
export const getVisibleGaps = (
  ...
  const referenceBounds = getMaximumGroups(referenceElements, elementsMap)
    ...
    .map(
      group =>
        getCommonBounds(group).map(bound => round(bound)) as unknown as Bounds,
    );
