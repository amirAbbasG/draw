...
import type { CaptureUpdateActionType, Scene } from "@excalidraw/element";
import type {
  Arrowhead,
  ElementsMap,
...
// ArrowHead icons
import {Range, RoundnessRange} from "../components/Range";
...
// Add this new action for the roundness slider
export const actionChangeRoundnessValue = register({
  name: "changeRoundnessValue",
  label: "Change roundness value",
  trackEvent: false,
    ...
    return {
      elements: changeProperty(elements, appState, el => {
        ...
        if (roundnessType === ROUNDNESS.ADAPTIVE_RADIUS) {
          // For adaptive radius: 0-50 pixels
          roundnessValue = value / 2; // Slider 0-100 -> Value 0-50
        } else {
          // For PROPORTIONAL_RADIUS and LEGACY: 0-0.5 ratio
          roundnessValue = value / 200; // Slider 0-100 -> Value 0-0.5
        }
      ...
      appState: {
        ...appState,
        currentItemRoundness: value > 0 ? "round" : "sharp",
      },
      captureUpdate: CaptureUpdateAction.IMMEDIATELY,
    };
  },
...
// Update the existing actionChangeRoundness to set proper default
export const actionChangeRoundness = register({
  ...
  perform: (elements, appState, value) => {
    return {
      elements: changeProperty(elements, appState, el => {
        ...
        const roundnessType = isUsingAdaptiveRadius(el.type)
            ? ROUNDNESS.ADAPTIVE_RADIUS
            : ROUNDNESS.PROPORTIONAL_RADIUS;

        // Set default to middle value (25px for adaptive, 0.25 for proportional/legacy)
        const defaultValue = roundnessType === ROUNDNESS.ADAPTIVE_RADIUS ? 25 : 0.25;
  ...
  PanelComponent: ({ elements, appState, updateData, app, renderAction }) => {
    ...
    // Check if any element has roundness enabled
    const hasRoundness = targetElements.some(el => el.roundness);

    const hasProportionalRoundness = targetElements.some(
        el => el.roundness?.type === ROUNDNESS.PROPORTIONAL_RADIUS
    );
    ...
    return (
        <fieldset>
          ...
          {/* Show slider when roundness is enabled */}
          {hasRoundness && !hasProportionalRoundness  && (
              <div style={{ marginTop: "0.5rem" }}>
                {renderAction("changeRoundnessValue")}
              </div>
          )}
        </fieldset>
    );
